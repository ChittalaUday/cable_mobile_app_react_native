...
type KpiSpec = { key: string; icon: IconSvgElement; tint: keyof typeof TINT; label: string; value: string; delta: number | null; series: number[]; tone: 'good' | 'bad' };

function kpiSpecs(data: AdminDashboard): KpiSpec[] {
  return [
    { key: 'customers', icon: UserMultiple02Icon, tint: 'orange', label: 'Total Customers', value: grouped(data.totalCustomers), delta: data.totalCustomersDelta, series: data.totalCustomersSeries, tone: 'good' },
    { key: 'active', icon: Wifi01Icon, tint: 'blue', label: 'Active Connections', value: grouped(data.activeConnections), delta: data.activeDelta, series: data.activeSeries, tone: 'good' },
    { key: 'inactive', icon: WifiDisconnected01Icon, tint: 'red', label: 'Inactive / Disconnected', value: grouped(data.inactiveConnections), delta: data.inactiveDelta, series: data.inactiveSeries, tone: 'bad' },
    { key: 'revenue', icon: IndianRupeeIcon, tint: 'green', label: 'Revenue (This Month)', value: rupees(data.revenueThisMonth), delta: data.revenueDelta, series: data.revenueSeries, tone: 'good' },
  ];
}
  ...
  return (
    <View className="flex-row flex-wrap gap-2.5">
      {kpiSpecs(data).map(kpi => (
        <Card key={kpi.key} className="min-w-[46%] flex-1 overflow-hidden pt-3">
          <View className="px-3">

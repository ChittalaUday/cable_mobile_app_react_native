...
const getLatestRelease = async () => {
  try {
    const repoData = await fetch(
      'https://api.github.com/repos/ChittalaUday/cable_mobile_app_react_native/releases/latest'
    );
...
const cloneLastTemplateRelease = async (projectName) => {
  ...
  const cloneStarter = `git clone -b ${latest_release} --depth=1   https://github.com/ChittalaUday/cable_mobile_app_react_native.git ${projectName}`;
